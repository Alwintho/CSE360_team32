...
/*******
 * 
 * This file has the username input verification functions from the stand-alone UserNameRecognizerConsoleTestbed project that 
	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 0: 
				...
				//CHANGE: The current character is checked against A-Z, a-z. If any are matched
				// 		  the FSM goes to state 1
				
				// A-Z, a-z, 0-9 -> State 1 CHANGE: A-Z, a-z
				if ((currentChar >= 'A' && currentChar <= 'Z' ) ||		// Check for A-Z
						(currentChar >= 'a' && currentChar <= 'z' ) /*||	// Check for a-z
						(currentChar >= '0' && currentChar <= '9' )*/) {	// CHANGED do not need to check 0-9 with 
					nextState = 1;										    // the new requirements
			...
			case 1: 
				...
				//  2: a period OR a minus sign OR a underscore that transitions to state 2 
				...
				// . -> State 2
				else if (currentChar == '.' || currentChar == '_' || currentChar == '-') {	//CHANGED now accept "_" and "-"
					nextState = 2;
		...
		case 1:
			...
			if (userNameSize < 4) {
				...
				userNameRecognizerErrorMessage += 
					...
					+ " you may put a \".\" , \"_\", or \"-\" between those valid characters..."; //CHANGED
				return userNameRecognizerErrorMessage;
			}
			else {
					// UserName is valid
