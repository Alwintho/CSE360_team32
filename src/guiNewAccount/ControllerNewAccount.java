...
public class ControllerNewAccount {
	...
	*/
	...
	protected static void doCreateUser() {
		...
		String passMessage = Password.evaluatePassword(password);
		String userMessage = Username.checkForValidUserName(username);
		
		if (!passMessage.isEmpty()) {
			ViewNewAccount.alertUsernamePasswordError.setTitle("You entered an erroneous password.");
			ViewNewAccount.alertUsernamePasswordError.setHeaderText("Your password didn't meet the following conditions.");
			ViewNewAccount.alertUsernamePasswordError.setContentText(passMessage);
			ViewNewAccount.alertUsernamePasswordError.showAndWait();
			return;
		} 
		if (!userMessage.isEmpty()) {
			ViewNewAccount.alertUsernamePasswordError.setTitle("You entered an erroneous username.");
			ViewNewAccount.alertUsernamePasswordError.setHeaderText("Your username didn't meet the following conditions.");
			ViewNewAccount.alertUsernamePasswordError.setContentText(userMessage);
			ViewNewAccount.alertUsernamePasswordError.showAndWait();
			return;
		} 
